...
// Used to view just a single random book.
//$cid = $_GET['cid'];
require XOOPS_ROOT_PATH . '/header.php';
OpenTable();
mainheader();
/*$q = "SELECT d.lid, d.cid, d.title, d.url, d.homepage, d.version, d.size, d.platform, d.price, d.logourl, d.status, d.date, d.hits, d.rating, d.votes, d.comments, t.description FROM ".$xoopsDB->prefix("gsdownloads_downloads")." d, ".$xoopsDB->prefix("gsdownloads_text")." t ORDER BY rand() LIMIT 1";
$result=$xoopsDB->query($q);
list($lid, $cid, $title, $url, $homepage, $version, $size, $platform, $price, $logourl, $status, $time, $hits, $rating, $votes, $comments, $description)=$xoopsDB->fetchRow($result);*/
$q = 'SELECT d.lid, d.cid, d.title, d.url, d.homepage, d.version, d.size, d.platform, d.price, d.logourl, d.status, d.date, d.hits, d.rating, d.votes, d.comments FROM ' . $xoopsDB->prefix('gsdownloads_downloads') . ' d ORDER BY rand() LIMIT 1';
$result = $xoopsDB->query($q);
[$lid, $cid, $title, $url, $homepage, $version, $size, $platform, $price, $logourl, $status, $time, $hits, $rating, $votes, $comments] = $xoopsDB->fetchRow($result);
$q2 = 'SELECT t.description FROM ' . $xoopsDB->prefix('gsdownloads_text') . " t WHERE lid=$lid";
