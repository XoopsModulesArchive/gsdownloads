...
if ('' != $_GET['show']) {
    ...
    $show = $gsdownloads_reviewsperpage;
}
if (!isset($_GET['min'])) {
    $min = 0;
} else {
...
OpenTable();
//Show Editorials
$result400 = $xoopsDB->query('SELECT editorialid, editorialuser, editorial, editorialhostname, editorialtimestamp FROM ' . $xoopsDB->prefix('gsdownloads_editorials') . " WHERE lid = $lid ORDER BY editorialtimestamp DESC");
$votes = $xoopsDB->getRowsNum($result400);
echo "<table width=100% border=0 class='bg2'><tr><td colspan=7><b>";
printf(_MD_REGUSEREDITORIALS, $votes);
echo "</b></td></tr></table>\n";
if (0 == $votes) {
    echo "<table width=100% class='bg2'><tr><td align=\"center\" colspan=\"7\">" . _MD_NOREGEDITORIALS . "<br></td></tr></table>\n";
}
while (list($editorialid, $editorialuser, $editorial, $editorialhostname, $editorialtimestamp) = $xoopsDB->fetchRow($result400)) {
    $formatted_date = formatTimestamp($editorialtimestamp);

    $editorial = $myts->displayTarea($editorial, 1);

    $editorialuname = XoopsUser::getUnameFromId($editorialuser);

    //echo "<table width=100% class='bg2'><tr><td colspan=7>\n";

    echo "<table width=100% border='0' cellspacing='1' cellpadding='2' bgcolor='#CC0000'><tr> <td colspan=1 valign='top' bgcolor='#FFFFFF' align=left> " . _MD_USER . ": <b>$editorialuname </b>&nbsp; &nbsp; &nbsp; &nbsp;" . _MD_DATE . ": <b>$formatted_date </b><br>$editorial</td></tr></table>\n";

    echo "<table width=100% border='0' cellspacing='0' cellpadding='0' class='bg2'><tr><td colspan=7 valign='top' align='right'><img src='images/flag.gif'></td></tr></table>\n";

    echo "<img src='shim.gif' width=1 height=4>";
}
//Show Reviews
// $result500=$xoopsDB->query( "SELECT reviewid, reviewuser, review, reviewhostname, reviewtimestamp FROM ".$xoopsDB->prefix("gsdownloads_reviews")." WHERE lid = $lid AND reviewuser != 0 ORDER BY reviewtimestamp DESC" );
$fullcountresult = $xoopsDB->query('SELECT COUNT(*) FROM ' . $xoopsDB->prefix('gsdownloads_votedata') . " WHERE lid=$lid");
[$numrows] = $xoopsDB->fetchRow($fullcountresult);
$q = 'SELECT reviewid, reviewuser, review, reviewhostname, reviewtimestamp FROM ' . $xoopsDB->prefix('gsdownloads_reviews') . " WHERE lid = $lid ORDER BY reviewtimestamp ASC";
$result500 = $xoopsDB->query($q, $show, $min);
############Start Here##########Query, Show X records, Start at 0 is the beginning of the recordset############
$votes = $xoopsDB->getRowsNum($result500);
echo "<table border=0 width=100% class='bg2'><tr><td colspan=4><b>";
...
if ($dispmax > 0) {
    echo "</b></td><td colspan=3 align=right>$dispmin-$dispmax " . _MD_OF . " <b>$numrows</b></td></tr></table>\n";
}
if (0 == $votes) {
    echo "<table border=0 widht=100% class='bg2'><tr><td align=\"center\" colspan=\"7\">" . _MD_NOREGREVIEWS . "<br></td></tr></table>\n";
}
//This is the review display code.
while (list($reviewid, $reviewuser, $review, $reviewhostname, $reviewtimestamp) = $xoopsDB->fetchRow($result500)) {
    ...
    $resulttot = $xoopsDB->query('SELECT rating FROM ' . $xoopsDB->prefix('gsdownloads_votedata') . " WHERE lid = $lid AND ratinguser=$reviewuser");

    [$totalrate] = $xoopsDB->fetchRow($resulttot);

    $resultcat = $xoopsDB->query('SELECT rating FROM ' . $xoopsDB->prefix('gsdownloads_votecat') . " WHERE lid=$lid AND ratinguser=$reviewuser");

    $totbar = ($totalrate / $gsdownloads_totrate) * 100;

    echo "<table border=0 width=100% class='bg2'><tr><td width=266 valign=top><table width=266><td></td><td width=58><font size=2><b>&nbsp;&nbsp;1</b></font></td><td align=right width=58><font size=2><b>$gsdownloads_totrate&nbsp;&nbsp;</b></font></td></tr><tr><td align=right width=100><font size=2><b>$gsdownloads_totname:</b></font></td><td colspan=2> <img src='images/leftbar1.gif' alt='$gsdownloads_totname: $totalrate'><img src='images/mainbar1.gif' height=15 width=$totbar alt='$gsdownloads_totname: $totalrate'><img src='images/rightbar1.gif' alt='$gsdownloads_totname: $totalrate'></td>";

    $q = 1;

    while (list($catrate) = $xoopsDB->fetchRow($resultcat)) {
        $catbar = ($catrate / $gsdownloads_maxrate) * 100;

        echo "<tr><td align=right width=100><font size=2>$gsdownloads_catname[$q]:</font></td><td colspan=2><img src='images/leftbar.gif' alt='$gsdownloads_catname[$q]: $catrate'><img src='images/mainbar.gif' height=16 width=$catbar alt='$gsdownloads_catname[$q]: $catrate'><img src='images/rightbar.gif' alt='$gsdownloads_catname[$q]: $catrate'></td><td><font size=2></font></td></tr>";
    ...
    echo '</td><td valign=top><table border=0 width=100%><tr><td align=right width=50%>' . _MD_USER . ": <b>$reviewuname </b></td><td align=right width=50%>" . _MD_DATE . ": <b>$formatted_date </b></td></tr><tr><td colspan=2>$review</td></tr></table>
</td></tr>";
...
echo '<tr><td align=center colspan=7><form action=index.php method=post>';
